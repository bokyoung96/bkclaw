...
def build_discord_payload(run_or_bundle_dir: str | Path) -> DiscordPayload:
    ...
    lines = ['[성과 요약] 📊', f'**전략:** {strategy_name}']
    if period:
    ...
    message = "\n".join(lines).strip() + "\n"
    return DiscordPayload(message_markdown=message, attachment_paths=[str(path) for path in attachments], metadata=metadata)
...
def _extract_metrics(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(metrics, dict):
        return metrics
    passthrough_keys = {'start','end','cagr','annual_return','annual_vol','volatility','sharpe','max_drawdown','mdd','calmar','avg_monthly_turnover','turnover','rebalance_count','final_holdings_count'}
    return {k: v for k, v in summary.items() if k in passthrough_keys}
...
def _metric_order() -> list[tuple[str, str]]:
    return [('CAGR','cagr'),('Annual Return','annual_return'),('Volatility','annual_vol'),('Sharpe','sharpe'),('MDD','max_drawdown'),('Calmar','calmar'),('Turnover','avg_monthly_turnover'),('Rebalance Count','rebalance_count'),('Holdings','final_holdings_count')]
...
def _collect_attachments(root: Path) -> list[Path]:
    candidates = [root/'strategy_bundle.png', root/'cumulative_return_and_drawdown.png', root/'return_distributions.png', root/'return_histograms.png', root/'cumulative_return_and_drawdown_lite.jpg', root/'return_distributions_lite.jpg']
    out=[]; seen=set()
